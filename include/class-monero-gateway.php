...
class Monero_Gateway extends WC_Payment_Gateway
{
    ...
    protected static function log($level, $message, array $context = array())
    {
        ...
        $context = array_merge(array('source' => self::$_id), $context);
    ...
    public static function should_enqueue_order_assets()
    {
    ...
    protected static function get_order_from_mixed($order)
    {
    ...
    protected static function current_request_has_valid_order_key($order)
    {
        $order = self::get_order_from_mixed($order);
        if (!$order) {
            return false;
        }

        $provided_key = isset($_GET['key']) ? wc_clean(wp_unslash($_GET['key'])) : '';
        if (!$provided_key && isset($_GET['order_key'])) {
            $provided_key = wc_clean(wp_unslash($_GET['order_key']));
        }

        return $provided_key && hash_equals((string) $order->get_order_key(), (string) $provided_key);
    }
    ...
    public static function admin_balance_info()
    {
        ...
        if (!isset($wallet_amount)) {
            ...
            self::log('error', 'Cannot connect to monero-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
    ...
    public function process_payment($order_id)
    {
        ...
        if(self::$confirm_type != 'monero-wallet-rpc') {
          ...
          if(isset($payment_id['address'])) {
            ...
            self::log('error', 'Could not create subaddress for order ' . $order_id);
          }
        }
        ...
        if($rate != 0)
            ...
            self::log('error', 'Impossible to retrieve price for order: ' . $order_id);
    ...
    public static function do_update_event()
    {
        ...
        $api_link = 'https://api.coingecko.com/api/v3/simple/price?ids=monero&vs_currencies=' . rawurlencode($currencies);
        $response = wp_remote_get($api_link, array(
            'timeout' => 20,
            'user-agent' => 'Monero WooCommerce Gateway/' . (defined('WC_VERSION') ? WC_VERSION : 'unknown'),
        ));
        $price = null;

        if (!is_wp_error($response) && 200 === wp_remote_retrieve_response_code($response)) {
            $price = json_decode(wp_remote_retrieve_body($response), true);
        }
        ...
        if(isset($price['monero']) && is_array($price['monero'])) {
            ...
            foreach($price['monero'] as $currency=>$rate) {
                ...
                $result = $wpdb->query($query);
              	if(!$result){
                    self::log('error', 'Impossible to write DB. Please check your DB connection or enable debugging.');
                }
            }
        }
        else{
             self::log('error', 'Unable to fetch prices from coingecko.com.');
        }
        ...
        if (is_array($height) && isset($height['height'])) {
            $height = (int) $height['height'];
        }
        set_transient('monero_gateway_network_height', (int) $height);
        ...
        foreach($pending_payments as $pending) {
            ...
            if($paid && $confirmed) {
                self::log('info', "Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                self::log('info', "Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::log('warning', "Payment has expired for order id $order_id and payment id $payment_id");
    ...
    protected static function check_payment_rpc($subaddress)
    {
        ...
        if(isset($address_index['index']['minor'])){
          ...
          self::log('error', 'Could not get address index of subaddress: ' . $subaddress);
          return $txs;
        }
    ...
    protected static function get_payment_details($order_id)
    {
        ...
        if (count($details)) {
            ...
            $order_age_seconds = $timestamp_now->getTimestamp() - $timestamp_created->getTimestamp();
            $order_expires_seconds = max(0, self::$valid_time - $order_age_seconds);
            ...
            if(self::$confirm_type == 'monero-wallet-rpc') {
                ...
                if ($address) {
                    ...
                    self::log('error', 'Merchant has not set Monero address');
    ...
    public static function get_payment_details_ajax() {
        ...
        if (!isset($_GET['order_id'])) {
            self::ajax_output(array('error' => '[ERROR] Missing order id'));
        }
        ...
        if (!$order) {
            self::ajax_output(array('error' => '[ERROR] Order not found'));
        }
        ...
        if (!$authorized && self::current_request_has_valid_order_key($order)) {
            $authorized = true;
        }

        if (!$authorized) {
            self::ajax_output(array('error' => '[ERROR] Unauthorized'));
        }
    ...
    {
        $order = self::get_order_from_mixed($post);
        if (!$order) {
            return;
        }
        if($order->get_payment_method() != self::$_id)
    ...
    public static function customer_order_page($order)
    {
        ...
        if (is_array($details)) {
            $details['order_key'] = $order->get_order_key();
        }
        if(!is_array($details)) {
            $error = $details;
