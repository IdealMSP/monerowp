...
function monero_init() {
    ...
    function monero_enqueue_scripts() {
        ...
        if (!Monero_Gateway::should_enqueue_order_assets()) {
            return;
        }

        if (Monero_Gateway::use_qr_code()) {
            wp_enqueue_script('monero-qr-code', MONERO_GATEWAY_PLUGIN_URL.'assets/js/qrcode.min.js', array(), null, true);
        }

        wp_enqueue_script('monero-clipboard-js', MONERO_GATEWAY_PLUGIN_URL.'assets/js/clipboard.min.js', array(), null, true);
        wp_enqueue_script('monero-gateway', MONERO_GATEWAY_PLUGIN_URL.'assets/js/monero-gateway-order-page.js', array('jquery'), '3.1.0', true);
        wp_enqueue_style('monero-gateway', MONERO_GATEWAY_PLUGIN_URL.'assets/css/monero-gateway-order-page.css', array(), '3.1.0');
    }
    ...
    if ( class_exists( 'Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
        require_once MONERO_GATEWAY_PLUGIN_DIR . 'include/blocks/class-monero-blocks-support.php';
        add_action(
            'woocommerce_blocks_payment_method_type_registration',
            function( $payment_method_registry ) {
                if ( class_exists( 'Monero_Gateway_Blocks_Support' ) ) {
                    $payment_method_registry->register( new Monero_Gateway_Blocks_Support() );
                }
            }
        );
    }
